...
class UnifiedMain(QWidget):
    ...
    def init_ui(self):
        ...
        logo = QLabel()
        pixmap = QPixmap(os.path.abspath(os.path.join(os.path.dirname(__file__), "../assets/logo.png")))
    ...
    def remove_rule(self):
        ...
        if port:
            rule["sport"] = int(port)
        success = self.rule_engine.remove_rule(rule)
        if success:
            self.output_box.append(f"❌ Rule removed: {rule}")
        else:
            self.output_box.append(f"⚠️ Rule not found: {rule}")
